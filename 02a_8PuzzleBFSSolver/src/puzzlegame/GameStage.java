...
public class GameStage {
	...
	// brute force solvers
	private BFS bfsSolver;
	private LinkedList<String> bfsSolution;
	private boolean isSolvedByBFS = false;
	...
	public GameStage (Stage stage) {
		...
		this.stage.setTitle("8-Puzzle Game with BFS");
...
//			System.out.println("YOU WIN!");		// uncomment for testing
	...
	private void displayMessage() {
		...
		if (this.isSolvedByBFS == false)
		gc.fillText("Solve by Breadth First Search".toUpperCase(), GameStage.WINDOW_WIDTH/2, GameStage.WINDOW_HEIGHT-90);
		else if (this.isSolvedByBFS == true) {
			if (this.isSolutionFinished) {
				gc.fillText("FINISHED !" + "\nPath Cost: " + this.bfsSolution.size() + "\n^ v ^", GameStage.WINDOW_WIDTH/2, GameStage.WINDOW_HEIGHT-70);

				System.out.println("====================");
				System.out.println("FINISHED !");
				System.out.println("Path:");
				for (int i=0; i<this.bfsSolution.size(); i++) {
					if (i<bfsSolution.size()-1)
					System.out.print(this.bfsSolution.get(i) + " -> ");
					else
					System.out.println(this.bfsSolution.get(i));
				}
				System.out.println("Path Cost: " + this.bfsSolution.size());
				System.out.println("====================");
			} else gc.fillText("Execute BFS Solution", GameStage.WINDOW_WIDTH/2, GameStage.WINDOW_HEIGHT-90);
		}
	...
	// sets buttons for brute force solver (bfs)
	void setInItButtons() {
		this.bfsButton = new Button("BFS");
		this.bfsButton.setMinWidth(200);
		this.bfsButton.setMinHeight(50);
		this.bfsButton.setTranslateX(100);
		this.bfsButton.setTranslateY(530);
		this.bfsButton.setStyle(
				"-fx-background-radius: 5em; "
		);
		this.root.getChildren().add(this.bfsButton);
	...
	void setButtonHandlers() {
		this.bfsButton.setOnMouseClicked(new EventHandler<MouseEvent>() {
			public void handle(MouseEvent e) {
				...
				bfsSolver = new BFS(gameBoard);
				bfsSolution = bfsSolver.solve().getPath();
				...
				displayMessage();
				bfsButton.setVisible(false);	// disables the bfs button
				nextButton.setVisible(true);	// enables the next button

				// for checking
				System.out.println("Solution:");
				System.out.print("\t");
				for (String a: bfsSolution) {
					System.out.print(a + " ");
				}
				System.out.println("");
				System.out.println("=====================");
				System.out.println("Path Cost:");
				System.out.println(bfsSolution.size());
				System.out.println("=====================");

				viewUsage(startTime, endTime, beforeUsedMem, bfsSolver.getUsedMem());
			}
		});
	...
	void translatePathIntoActions () {
		this.gameBoard = this.bfsSolver.getResult(this.gameBoard, this.bfsSolution.get(nextIndex));

		// for checking
		System.out.println("====================");
		System.out.println("Action Performed: " + this.bfsSolution.get(nextIndex));

		// disable the nextbutton if finished
		if (nextIndex == this.bfsSolution.size()-1) {
	...
	void exportSolution () {
		...
		for (String action : this.bfsSolution) {
			fileContent += action + " ";
		}
