...
public class GameStage {
	...
	private LinkedList<String> aStarSolution;
	private boolean isSolvedByAStar = false;
	...
	public GameStage (Stage stage) {
		...
		this.stage.setTitle("8-Puzzle Game w/ A* Search");
...
//			System.out.println("YOU WIN!");		// uncomment for testing
	...
	private void displayMessage() {
		...
		if (this.isSolvedByAStar == false)
		gc.fillText("Solve by A* Search".toUpperCase(), GameStage.WINDOW_WIDTH/2, GameStage.WINDOW_HEIGHT-90);
		else if (this.isSolvedByAStar == true) {
			if (this.isSolutionFinished) {
				gc.fillText("FINISHED !" + "\nPath Cost: " + this.aStarSolution.size() + "\n^ v ^", GameStage.WINDOW_WIDTH/2, GameStage.WINDOW_HEIGHT-70);

				System.out.println("====================");
				System.out.println("FINISHED !");
				System.out.println("Path:");
				for (int i=0; i<this.aStarSolution.size(); i++) {
					if (i<aStarSolution.size()-1)
					System.out.print(this.aStarSolution.get(i) + " -> ");
					else
					System.out.println(this.aStarSolution.get(i));
				}
				System.out.println("Path Cost: " + this.aStarSolution.size());
				System.out.println("====================");
			} else gc.fillText("Execute A* Solution", GameStage.WINDOW_WIDTH/2, GameStage.WINDOW_HEIGHT-90);
		}
	...
	// sets buttons for brute force solver (A*)
	void setInItButtons() {
		this.aStarButton = new Button("A*");
		this.aStarButton.setMinWidth(120);
		this.aStarButton.setMinHeight(50);
		this.aStarButton.setTranslateX(75);
		this.aStarButton.setTranslateY(530);
		this.aStarButton.setStyle(
				"-fx-background-radius: 5em; "
		);
		this.root.getChildren().add(this.aStarButton);

		this.openFileButton = new Button("New Input");
		this.openFileButton.setMinWidth(120);
		this.openFileButton.setMinHeight(50);
		this.openFileButton.setTranslateX(200);
		this.openFileButton.setTranslateY(530);
		this.openFileButton.setStyle(
				"-fx-background-radius: 5em; "
		);
		this.root.getChildren().add(this.openFileButton);
	...
	void setButtonHandlers() {
		this.aStarButton.setOnMouseClicked(new EventHandler<MouseEvent>() {
			public void handle(MouseEvent e) {
				...
				aStarSolver = new AStar(gameBoard);
				aStarSolution = aStarSolver.solve().getPath();
				LocalDateTime endTime = LocalDateTime.now();	// end time of execution


				isSolvedByAStar = true;
				exportSolution();

				gridPane.setDisable(true);						// disable the grid pane for clicking
				displayMessage();
				aStarButton.setVisible(false);					// disables the aStar button
				openFileButton.setVisible(false);					// disables the openFileButton button
				nextButton.setVisible(true);					// enables the next button

				// for checking
				System.out.println("Solution:");
				System.out.print("\t");
				for (String a: aStarSolution) {
					System.out.print(a + " ");
				}
				System.out.println("");
				System.out.println("=====================");
				System.out.println("Path Cost:");
				System.out.println(aStarSolution.size());
				System.out.println("=====================");

				viewUsage(startTime, endTime, beforeUsedMem, aStarSolver.getUsedMem());					// check usage of resources
			}
		});

		this.openFileButton.setOnMouseClicked(new EventHandler<MouseEvent>() {
			public void handle(MouseEvent e) {
				openFile();
			}
		});
	...
	// ===========================================================================================
	// opens a new puzzle.in file for input
	void openFile() {
		...
		String fileDirectory = System.getProperty("user.dir");
		fileDirectory += "\\src\\puzzlegame\\";
		fileChooser.setInitialDirectory(new File(fileDirectory));

		// adds an extension filter
		ExtensionFilter extensionFilter = new ExtensionFilter("Puzzle Input Files", "*.in");
		fileChooser.getExtensionFilters().add(extensionFilter);

		// opens the dialog box
		File selectedFile = fileChooser.showOpenDialog(this.stage);

		// if there's a read file
		if (selectedFile != null) {
			this.readInputFile(selectedFile);

			// reset variable values/properties
			this.isGameOver = false;
			this.isSolutionFinished = false;
			this.aStarButton.setVisible(true);
			this.nextIndex = 0;
	...
	void translatePathIntoActions () {
		this.gameBoard = this.aStarSolver.getResult(this.gameBoard, this.aStarSolution.get(nextIndex));

		// for checking
		System.out.println("====================");
		System.out.println("Action Performed: " + this.aStarSolution.get(nextIndex));

		// disable the nextbutton if finished
		if (nextIndex == this.aStarSolution.size()-1) {
			this.isSolutionFinished = true;
			this.nextButton.setVisible(false);
			this.openFileButton.setVisible(true);
			this.openFileButton.setTranslateX(270);
			this.openFileButton.setTranslateY(540);
		}
	...
	void exportSolution () {
		...
		for (String action : this.aStarSolution) {
			fileContent += action + " ";
		}
